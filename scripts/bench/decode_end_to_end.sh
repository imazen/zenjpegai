...
NPROC=$(nproc)
# Thread counts to sweep (reference --threads / zenjpegai rayon pool); "1" means single-thread.
...
for s in $STREAMS; do
  ...
  for th in $THREADS; do
    # th=1 uses --single-thread (no pool); th>1 runs the rayon pool capped at th threads.
    ...
    start=$(date +%s%N)
    RAYON_NUM_THREADS=$pool ZENJPEGAI_MODELS=$REF/models nice -n 19 "$BIN" decode "$bits" "$TMP/zen.png" $mode
    end=$(date +%s%N)
    out=$(RAYON_NUM_THREADS=$pool ZENJPEGAI_MODELS=$REF/models nice -n 19 "$BIN" decode "$bits" "$TMP/zen.png" --repeat $((RUNS + 1)) $mode 2>&1)
    r=0
    while read -r ms; do
