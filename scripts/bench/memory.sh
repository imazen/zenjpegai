...
# MODE=decode (default): `--discard`, the library's decode alone. MODE=cli: decode + PNG write.
MODE=${MODE:-decode}
# POOL_MB: cap of the recycled-buffer pool (`--pool-mb`); unset = the default 1 GiB.
EXTRA=""; [ "$MODE" = decode ] && EXTRA="--discard"
[ -n "${POOL_MB:-}" ] && EXTRA="$EXTRA --pool-mb $POOL_MB"
...
for s in $STREAMS; do
  for mode in "" "--single-thread"; do
    ...
    out=$TMP/ht.$LABEL.$MODE.$s.$th
    rm -f "$out".zst
    ZENJPEGAI_MODELS=$REF/models nice -n 19 heaptrack -o "$out" "$BIN" decode "$VEC/$s/stream.bits" "$TMP/o.png" $mode $EXTRA >/dev/null 2>&1
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$LABEL" "$MODE" "$s" "$th" "$(tobytes "$peak")" "$(tobytes "$rss")" "$calls" "$temp"
  done
done
