...
`enc_img30_bop_m1_b0_{threads8, rvs, rvsonly, grfsonly, lsbs, qmap, qmap_rvs}` and
...
| `z_hat`, `skip_scale_log`, `scale_log`, cube flags | identical on all seventeen, both components |
| stream bytes vs the reference encoder's | identical on thirteen of seventeen; the other four are the same length |
...
Not started (encoder): the post-filters on the encode side, and the rate matcher's
...
- ~~**E6 Chroma-subsampled, 10-bit and YUV sources.**~~ **Done 2026-09-18**: `SourceImage` /
  `SourceMeta` / `read_yuv`, `encoder::resample::resize_bilinear` (bit-identical to the
