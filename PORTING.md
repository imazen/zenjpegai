...
## Encoder parity (measured 2026-09-17, 560x888 test image 00030, eighteen fixed-model encodes)
...
| `z_hat`, `skip_scale_log`, `scale_log`, cube flags | identical on all eighteen, both components |
| stream bytes vs the reference encoder's | identical on fourteen of eighteen; the other four are the same length |
...
Not started (encoder): the EFE non-linear post-filter on the encode side.
