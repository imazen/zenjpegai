...
bop_m0_bm1069, hop_m3_bm1069}`, `enc_img01_bop_m1_b0` (2096x1400, 6 analysis tiles) and
`enc_img30_bop_m1_b0_{threads8, rvs, rvsonly, grfsonly, lsbs}`
(`make_reference_streams.sh encoder`).

| what | result |
| --- | --- |
| colour pre-processing vs the reference's own analysis inputs | bit-identical (every sample, every analysis tile) |
| `z_hat`, `skip_scale_log`, `scale_log`, cube flags | identical on all thirteen, both components |
| stream bytes vs the reference encoder's | identical on twelve of thirteen |
...
| our streams through the **reference decoder** vs through ours | 51-59 of 1,491,840 samples differ (504 of 8,803,200 at 2096x1400), all by 1: the decoder's own float-network gap, table above |
