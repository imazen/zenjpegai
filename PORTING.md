...
| `encoder` | `ccs_sgmm_tool.py::compress`, `sep_chan_tool.py::compress`, `common_modules.py::{compress, _compress_z, encoder_get_scales, _compress_ar_scale, encoder_skip_and_cubeflag_for_tiles, encode, encode_z, encode_y, _ac_encode_y, _ac_encode_z}`, `image.py`/`colorspace.py` (`to_YUV_`, `pad_`, `pixel_unshuffle`) | ported for: a fixed model and operating point (SOP / BOP / HOP), 8-bit RGB 4:4:4 sources up to 120 MP, any number of analysis tiles, one region, one ANS thread per substream, tools off. `Encoder::encode(rgb, EncodeParams { model_id, beta_displacement_log, op })` and `Encoder::encode_to_bpp(rgb, bpp, op)`; CLI `zenjpegai encode [--model N --beta-disp B | --bpp R]`. Coding tools on the encode side: RVS, GRFS (the encoder derives the gain flags, `analyzeCWG`), LSBS, 1..16 ANS threads per substream, and region partitioning (dependent and independent; the grid follows `calc_numHor_numVer_regions`). **Not ported: quality maps and the post-filters on the encode side, chroma-subsampled and 10-bit sources, `num_chs` below the model's channel count; the rate matcher's likelihood estimator (`ECLibLH`) and its `hyperopt` UV search (`src/encoder/rate.rs` says what is done instead)** | `tests/encode_ref.rs` on fifteen fixed-model reference encodes (models 0..3, SOP / BOP / HOP, beta displacement -1069..+400, 2096x1400, one per coding tool and one per region mode): colour pre-processing bit-exact against the reference's own analysis inputs; `z_hat`, `skip_scale_log`, `scale_log` and the cube flags equal the reference encoder's exactly; eleven of fifteen streams byte-identical to the reference's and the other four the same length; every stream decodes with `zenjpegai::Decoder` and with the reference decoder |
...
`enc_img30_bop_m1_b0_{threads8, rvs, rvsonly, grfsonly, lsbs}` and
`enc_img01_bop_m1_b0_{depregions, indregions}` (`make_reference_streams.sh encoder`).

| what | result |
| --- | --- |
| colour pre-processing vs the reference's own analysis inputs | bit-identical (every sample, every analysis tile) |
| `z_hat`, `skip_scale_log`, `scale_log`, cube flags | identical on all fifteen, both components |
| stream bytes vs the reference encoder's | identical on eleven of fifteen; the other four are the same length |
...
| our streams through the **reference decoder** vs through ours | 43-59 of 1,491,840 samples differ (435-504 of 8,803,200 at 2096x1400), all by 1: the decoder's own float-network gap, table above. Region streams use the patched decoder, because the stock one mis-decodes every region stream including its own |
...
1. **Encoder, the rest of it.** The encoder is ported through analysis tiling and rate matching
   ...
   1. **The quality map on the encode side** - the one coding tool the decoder supports and the
      encoder does not. `decoder::entropy::ComponentScales::quantize` returns `None` as soon as a
      quality map is on; the encoder also has to build the map (`quality_map/quality_map.py`:
      `qp_map_type 3` reads a mask file, the other generators crash on this code path) and ANS-code
      it into SOQ. Gate: `enc_fixed` vectors like the `qmap` set of
      `make_reference_streams.sh`, plus the patched decoder (the stock one cannot read quality
      maps either).
