...
| `weights::packed` | (no upstream source: a container of our own) | done: `ZJM1` (one checkpoint: name / dtype / shape / offset table + 64-byte aligned little-endian tensor bytes) read through the same `Checkpoint` accessors as `.pth`; `ZJB1` bundle of such files + the upstream licence text, served zero-copy as a `ModelSource` (`PackedBundle`, mergeable); `Recorder` + `zenjpegai pack-models` keep only the tensors the loaders look up (`model::with_checkpoint` reports them). Lossless (the f16 sibling is the next row). Bundle sizes per (model, operating point): SOP 15.05 MB, BOP 16.22 MB, HOP 28.76 MB (the `.pth` files they come from: 22..128 MB); split: common part of a model 12.36 MB, synthesis part SOP 2.69 / BOP 3.86 / HOP 16.40 MB; all 4 models x 3 points 141 MB from 353 MB. gzip -9 saves only 9 % (float weights). **Post-filter checkpoints are not packed yet: their loaders must go through `model::with_checkpoint`** | `tests/packed_bundle.rs`: SOP / BOP / HOP decodes from a bundle (whole and split) equal the `.pth` decodes byte for byte; repacking a bundle reproduces it; damaged bundles are rejected |
| `weights: f16 storage` (`weights::f16`, `ZJB2`) | (no upstream source: a storage variant of `weights::packed`) | done, **measured-and-rejected for shipped bundles**: `pack-models --f16` writes `ZJB2` — the `ZJB1` envelope, `ZJM1` members with every f32 network weight as IEEE-754 f16 (`weights::f16`: pure-integer round-to-nearest-even, all 65 536 halfs verified as round-trip fixed points); integer tensors and `vr_vec.c` are copied byte for byte so the entropy stage stays bit-exact; `Checkpoint::f32` upcasts at load so the compute path and every SIMD tier see ordinary f32. `ZJB1`/f32 remains the shipped default everywhere | `tests/f16_weights.rs` + `benchmarks/f16_weights_2026-09-19.md`: BOP bundle 16.22 → 8.62 MB (brotli page payload 14.57 → 7.31 MB), integer stage bit-identical, all tiers/threads bit-identical — **but all 43 real vectors fail the `≤1 LSB in <1/5000` gate** (1.1–8.0 % differ, 56–397× over), both common-only and synthesis-only hybrid splits fail too, and adversarial sources deviate up to 20 LSB; ~2.2 % of the weights flush to f16-zero (10.8 % of the HOP decoder). Not used by any shipped artefact |
...
| `gpu/` (`zenjpegai-gpu`, separate workspace member; the core crate does not depend on wgpu) | same sources as `model::synthesis` / `model::attention`; runs them as WGSL compute shaders through wgpu 30 | ported: SOP, BOP, HOP synthesis (luma + chroma), synthesis tiling and independent regions, `GpuDecoder` (CPU entropy / latent stage and output stage, GPU synthesis), readback-free `rgba8unorm` presentation; builds for wasm32. **Runs in a browser (verified on hardware, 2026-09-18 — row `B1`); no f16**; two tuning passes done (second pass: HOP 560x888 device 226 -> 103 ms, hot convs at ~25-37% of f32 peak, was ~8% — `gpu/README.md` "Status", work-queue row `B3`) | `gpu/tests/kernels.rs`: every kernel vs `nn::reference` / `nn::fast::math`, max relative error 1.7e-6 (conv, transposed conv), 2.5e-7 (attention), exact copies / shuffles. `gpu/tests/decode_ref.rs`: planes vs reference 3.4e-4 (SOP), 3.8e-4 (BOP), 3.3e-4 (HOP), 6.3e-4 / 6.3e-4 (2096x1400, 6 tiles / 12 tiles in 2 independent regions), bound 3e-3; 8-bit output differs by 1 in 47 / 59 / 33 of 1,491,840 (SOP / BOP / HOP) and 345 / 368 of 8,803,200 (2096x1400, 6 / 12 tiles) samples; vs the CPU engine planes differ by at most 8.7e-4; the `rgba8unorm` presentation texture differs from the CPU output stage in 27 of 1,491,840 samples, all by one step (it was 46,533 until the shader rounded half-to-even itself instead of trusting the driver's float-to-unorm conversion — llvmpipe had agreed, NVIDIA did not); the GPU-quantised readback (`emit_output`, 8/10-bit YUV or RGB packed `u16`) differs from the CPU output stage on the same GPU planes by 38 / 37 / 27 of 1,491,840 and 160 / 177 of 8,803,200 samples, all one step. All measured on an **RTX 2080** (NVIDIA 580.178.04, `ZENJPEGAI_GPU_ADAPTER=GeForce just gpu-test`), feature `gpu-tests`. The counts moved slightly under pass-2 tuning (3ced0e63): `conv_tiled` sums input-channel `ICH` chunks over all taps rather than all channels per tap — a different association for `KH > 1`, measured, never loosened; every other kernel keeps the per-output order. Timings: `benchmarks/gpu_decode_2026-09-17_rtx2080.{tsv,meta}`, `gpu_decode_2026-09-18_rtx2080{,_kernels}.tsv`, `gpu_profile_2026-09-17_rtx2080.{tsv,meta}`, `gpu_kernels_2026-09-18_rtx2080.tsv`, reference software on the same card `gpu_reference_2026-09-17.{tsv,meta}` |

| Fuzzing (`fuzz/`, cargo-fuzz + libFuzzer) | (hardening; no upstream source) | three targets on the decoder's untrusted-input surface: `container_headers` (marker scan, `Codestream::parse`, `Headers`/`header::parse` incl. TON/RDI/UDI, region + thread split helpers), `entropy_stage` (`decode_entropy_stage*` on arbitrary bytes against a fixed synthetic zero-weight `ZJM1` model, with a counting `Stop`), `decode_full` (`Decoder::decode_with` under tight `Limits` — max pixels / dimensions / input bytes / memory — and cancellation, on arbitrary bytes and mutated real streams). Synthetic checkpoints exercise the whole pipeline without committing weights. Dictionary `fuzz/zenjpegai.dict` (markers, model magics, ue/se patterns, real header prefixes), seed corpus `fuzz/seeds/` (47.9 KB: smallest reference streams + truncations), stable replay suite `tests/fuzz_regression.rs` walking `fuzz/regression/*` through the same entry points. `just fuzz`; CI job builds the targets on nightly and runs the replay on stable; corpora/artifacts sync to `/mnt/v/fuzzes/zenjpegai/` via `~/work/zen-workspace/fuzz-sync.sh` | first campaign 2026-09-20: `container_headers` 421 s, 18.7 M execs, peak RSS 738 MB, no crash / timeout / OOM / slow unit; `entropy_stage` and `decode_full` run with the same settings — findings in `CHANGELOG.md` |
...
- ~~**B1** WebGPU synthesis in the worker/polyfill/demo behind feature detection~~ **Done
  ...
  **B3** GPU kernels: mostly done (b3gpu + b3gpu2, 2026-09-18). **Still missing:** `shader-f16`
  ...
  bound because f16 accumulation changes the quantisation counts; not attempted). Done:
  bounded pool/workspace retention (`release` + first-`fit`-of-run shrink; a 4096x4096 run then
  560x888 drops the workspace 568 -> ~195 MiB — and the "small picture runs slow after a big
  one" symptom turned out to be GPU clock ramping between CPU-interleaved benchmark rounds,
  not buffer reuse), GPU-side output conversion with packed-u16 readback (decode wall −20 to
  −29%), `enough` cancellation between submits + `max_channels`, and a second kernel pass:
  workgroup-staged input halos + weight staging for stride-1/2 convolutions, channel-contiguous
  depthwise workgroups, flattened `elu_gate`, channel-group constants baked into the conv
  pipelines. HOP 560x888 device time 226 -> 103 ms (−54%), HOP 4096x4096 8919 -> 4045 ms,
  decode BOP 560x888 22.8 -> 17.4 ms, HOP 326 -> 109 ms, img01 2096x1400 164 -> 93 ms; the hot
  conv kernels now run at ~25-37% of f32 peak (was ~8%), `elu_gate` at 88% of bandwidth peak,
  depthwise at 30%. Two-pixel-per-invocation convolutions measured slower (register pressure)
  and were reverted. A plan-cache bug found by the bench's `plans_built` assertion — the pool's
  shrink/grow ping-pong between heterogenous tile sizes invalidated every cached plan mid-run —
  is fixed by bumping the pool generation only when buffers are actually dropped, and by
  allowing shrink only at the first `fit` of a run; a second gap found in review — the plan
  cache's `(generation, pool.generation)` stamp could collide across two separate `Workspace`s
  whose pooled buffers differ — is closed by a unique `Workspace` id in the stamp. Timings:
  `benchmarks/gpu_decode_2026-09-18_rtx2080_kernels.tsv` (full sweep; rows right after a
  multi-second CPU phase still carry the downclocked-GPU artifact — NVIDIA 580.178.04 ramps to
  1.9 GHz only under sustained load, no permission to lock clocks),
